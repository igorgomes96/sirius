...
export class PedidosApiService {
  ...
  getAggregatedByData(data: Date): Observable<ItemCardapio[]> {
    return this.httpClient.get<ItemCardapio[]>(this.url + '/agrupados', { params: { data: data.toString() } }).pipe(
      retry(3),
      take(1));
  }
  ...
  put(id: string, pedido: Pedido): Observable<void> {
    return this.httpClient.put<void>(this.url + `/${id}`, pedido).pipe(
      retry(3),
      take(1));
  }
  ...
  confirmacaoPedido(id: string, pedido: Pedido): Observable<void> {
    return this.httpClient.put<void>(this.url + `/${id}/confirmacao`, pedido).pipe(
      retry(3),
      take(1));
  }
  ...
  deleteItem(id: string, idItem: string): Observable<void> {
    return this.httpClient.delete<void>(this.url + `/${id}/item/${idItem}`).pipe(
      retry(3),
      take(1));
  }

  additem(id: string, item: ItemCardapio): Observable<ItemCardapio> {
    return this.httpClient.post<ItemCardapio>(this.url + `/${id}/item`, item).pipe(
      retry(3),
      take(1));
  }

  addItens(id: string, itens: ItemCardapio[]): Observable<any> {
    return this.httpClient.post<any>(this.url + `/${id}/itens`, itens).pipe(
      retry(3),
      take(1));
  }

  getImpressoes(): Observable<any[]> {
    return this.httpClient.get<any[]>(this.url + `/impressoes`).pipe(
      retry(3),
      take(1));
  }
